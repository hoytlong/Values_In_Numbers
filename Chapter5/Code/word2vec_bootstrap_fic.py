"""
word2vec_bootstrap_fic.py: build multiple word2vec models with bootstrap samples
usage: python3 word2vec_bootstrap_fic.py --embed_dim 100 --num_bootstrap 20
"""
...
folder_path = os.path.abspath("word2vec_bootstrap_fic.py" + "/../../")

### some pre-processing functions

def punc_cleaner(raw):
    raw = re.sub(r'[一二三四五六七八九十]+[百千万]*', '', raw)   #delete all numbers
    raw = re.sub(r'[×‥・○◎]+', '', raw)   #delete these punctuation marks
    raw = re.sub(r'[Ａ-Ｚ]+', '', raw)     #delete full-width letters
...
def get_char_names():
    char_names = []
    char_names_file = folder_path + "\\WordLists\CharNames.txt"
...
def main():
    ...
    corpus_path = folder_path + '\\Corpora\FictionW2VPreProcessed\\'  #store pre-processed texts here
    input_path = folder_path + '\\Corpora\UnidicLemmaMerged\\'   #previously lemmatized texts are here; racial identifiers should be merged
    
    # read in metadata spreadsheet for the corpus you are working with
    meta_file = folder_path + '\\Data\Fiction_Meta.xlsx'
	df = pd.read_excel(meta_file, sheetname='metadata')
    df = df.reset_index(drop=True)
    
    input_files = df.WORK_ID.tolist()
    #input_files = [file for file in os.listdir(input_path) if file.endswith('.txt')]
    
    char_names = get_char_names()
    for i in range(len(input_files)):
        file = str(input_files[i]) + '.txt'
        with open(os.path.join(input_path, file), 'r', encoding = 'utf-8') as f:
            raw = f.read()
            raw = punc_cleaner(raw)
            raw = bracket_cleaner(raw)
            raw = char_cleaner(raw, char_names)  #strip proper names
            with open(os.path.join(corpus_path, file), 'w', encoding="utf-8") as g:
                g.write(raw)
            g.close()    
    
    file_name = folder_path + '\\results_fic_bootstrap\word2vec_bootstrap\model_'
    ...
    model = gensim.models.word2vec.Word2Vec(words_by_sentence, size=embed_dim, window=10, min_count=5, sg=1, alpha=0.025, iter=5, batch_words=10000)
    model.wv.save_word2vec_format(folder_path + '\\results_fic_bootstrap\word2vec.txt')
	
	#clean "space" character in bootstrap files; failure to perform this operation will prevent "Cluster_Detection" code from running properly
	
	#point to folder where bootstrap files are stored
	model_dir = folder_path + '\\results_fic_bootstrap\word2vec_bootstrap\\'
