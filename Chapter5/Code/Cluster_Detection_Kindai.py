...
Usage:
    python3 Cluster_Detection_Kindai.py --thres 0.69 --num_test 5000 --window_size 20 --alpha 0.01 --overlap_percent 0.3
...
def main():
    ...
    folder_path = os.path.abspath("Cluster_Detection_Kindai.py" + "/../../")
	
    # set up file names and directory
    word_vec_bootstrap_folder = folder_path + '\\results_kindai_bootstrap\word2vec_bootstrap\\' # folder containing bootstrap results
    word_vec_file =  folder_path + '\\results_kindai_bootstrap\word2vec.txt' # word2vec fit on the original corpus
    corpus_dir = folder_path + '\\Corpora\KindaiW2VPreProcessed\\' # corpus folder; point to the pre-processed w2v corpus
    markers = ['朝鮮人', '中国人', '西洋人', '日本人', '黒人', '部落民', '土人', '東洋人', '外国人']
    result_file = folder_path + '\\results_kindai_bootstrap\kfree_thres%.2f_numtest%s_window%s_alpha%.2f_overlap_percent%.2f.txt' # output file name format
